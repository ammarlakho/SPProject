        ...
        //                exec
                            ...
                            if (j>=1) {
                                ...
                                if(pidRun == 0) {
                                    ...
                                    MyProcess newP = MyProcess(pid, args[0], start_time);

                                    int pSize = write(pipeProcess[1], &newP, sizeof(MyProcess));
                    ...
                    //                    total time it ran for
                                        ...
                                        ansL += sprintf(rowP2, "End: %s, T(Elapsed): %ds, T(Execution): %d, Active:%s\n", ctime(&end_time), elapsed_time, execution_time, active);
                                        strcat(ansS, rowP1);
                                        strcat(ansS, rowP2);
                                    }
                                }
            ...
            //        Send result to client
                    ...
                    if(type == 0) {
                        for(int i=0; i<MAX_ACTIVE; i++) {
                            int a = kill(activeList[i].pid, SIGTERM);
                        }
                        exit(0);
                    }
